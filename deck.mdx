...
2.  日常开发中使用的框架，脚手架，打包工具，再到编辑器的代码补全，代码格式化等功能

---

## AST（bstract syntax tree）抽象语法树
...
<CodeSurferColumns themes={[vsDark, github]} sizes={[1,3]}>
...
```js 3[11:14] file=./src/ast/literal/index.js
...
```js 7[20:25] file=./src/ast/literal/index.js

```

```json5 594[21:45] showNumbers file=./src/ast/literal/index.json title="type为Literal字面量"
...
```json5 38,54 showNumbers file=./src/ast/expression/index.json title="type为Expression" subtitle="表达式执行完以后有返回值，而这个返回值能赋值给一个变量，这是和语句 (statement) 的区别"
...
```js 2[1:6],9[6:8] file=./src/ast/expression/index.js
...
```json5 776 showNumbers file=./src/ast/expression/index.json title="单独的{}是BlockStatement"
...
1. @babel/parser
2. @babel/traverse
3. @babel/types (用来快速创建一个 ast 节点、判断节点类型)
4. @babel/generator
5. @babel/core
...
```js 1[12,13],2,3[10:12],4[12,13] title="dt值改为100、去除console、add改为sum"
const dt = 1;
console.log(dt);
function add(x, y) {
  return x + y;
...
- [token 在线查看：astexplorer](https://esprima.org/demo/parse.html#)
